...
fn open_github_issue(error: &str) {
    let title = encode("<issue name> Bug Report");
...
",
    ...
    let url = format!(
        "https://github.com/Tamino1230/RustConnect/issues/new?title={}&body={}",
        title,
        body
    );
...
impl eframe::App for ScreenClientApp {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
            ...
            egui::Window::new(format!("⚠️ {}", err.title))
                ...
                .show(ctx, |ui| {
                    ui.vertical_centered(|ui| {
                        ...
                        ui.add_space(12.0);
                        if ui.button("Open Issues").clicked() {
                            open_github_issue(&err.message);
                        }
        ...
        if ctx.input(|i| i.key_pressed(egui::Key::F12)) {
            state_guard.errors.push_back(UiError {
                title: "Test Error".into(),
                message: "Testing the error popup.".into(),
            });
        }
